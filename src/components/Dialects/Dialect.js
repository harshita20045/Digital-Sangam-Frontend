...
function Dialect() {
  ...
  const filteredDialects = allDialects.filter((dialect) => {
    const statusApproved = dialect.status === "approved";
    const languageMatches =
      selectedLanguage === "All" ||
      dialect.language?.language === selectedLanguage ||
      dialect.language === selectedLanguage;
    const searchMatches =
      dialect.word.toLowerCase().includes(searchQuery.toLowerCase()) ||
      (dialect.meaning?.english &&
        dialect.meaning.english.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (dialect.meaning?.hindi &&
        dialect.meaning.hindi.toLowerCase().includes(searchQuery.toLowerCase()));

    return statusApproved && languageMatches && searchMatches;
  });
